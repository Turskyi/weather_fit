...
class WeatherTileService : androidx.wear.tiles.TileService() {
    override fun onTileRequest(
        ...
        android.util.Log.d(
            "WeatherFitTile", "onTileRequest: Loading tile layout",
        )
        ...
        val weather = try {
            ...
            android.util.Log.e(
                "WeatherFitTile",
                "onTileRequest: Failed to load data", e,
            )
            WeatherTileData.empty()
        }

        val tile = androidx.wear.tiles.TileBuilders.Tile.Builder()
            .setResourcesVersion("${weather.location}_${weather.temperature}_${weather.imagePath.isNotBlank()}")
            .setTileTimeline(
    ...
    override fun onTileResourcesRequest(
        ...
        val weather = WeatherTileData.from(this)
        val builder =
            androidx.wear.protolayout.ResourceBuilders.Resources.Builder()
                .setVersion("${weather.location}_${weather.temperature}_${weather.imagePath.isNotBlank()}")

        if (weather.hasImage) {
            val file = File(weather.imagePath)
            if (file.exists()) {
...
private fun weatherBackgroundColor(code: Int, isNight: Boolean): Int {
    return when {
        code == 0 || code == 800 -> if (isNight) Color.rgb(
            26,
            26,
            51
        ) else Color.rgb(255, 153, 0)
